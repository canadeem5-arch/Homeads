...
/* ==========================================================================
   ADVANCED SCROLL ANIMATION OBSERVER (AOS)
   ========================================================================== */
document.addEventListener('DOMContentLoaded', function() {
  ...
  const aosObserver = new IntersectionObserver(function(entries, observer) {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('aos-animate');
        // observer.unobserve(entry.target); // Uncomment if we only want it to animate once
      } else {
        // Optional: Remove class when out of view so it animates again when scrolling back up
